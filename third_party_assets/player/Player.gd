...
onready var flashlight = get_node("Camera/Flashlight")
...
func _input(event):
	if Input.is_action_pressed(flashlight_action):
		if flashlight.light_energy == 0:
			...
			GlobalWorld.flashlight_status = true
		else:
			flashlight.light_energy = 0;
			GlobalWorld.flashlight_status = false
...
func _on_Safe_Space_exited(body_id, body, body_shape, area_shape):
	GlobalWorld.in_safe_space = false

func _on_Safe_Space_entered(body_id, body, body_shape, area_shape):
	GlobalWorld.in_safe_space = true
